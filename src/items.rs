...
pub enum PlayerTemporaryBuff { HealthRegen { rate: f32, duration_secs: f32 }, }
...
pub enum ItemEffect {
    ...
    OnThoughtFragmentHitExplode { chance: f32, explosion_damage: i32, explosion_radius: f32, explosion_color: Color, },
    OnPlayerHitRetaliate { chance: f32, retaliation_damage: i32, retaliation_radius: f32, retaliation_color: Color, },
    OnEnemyKillTrigger { chance: f32, effect: PlayerTemporaryBuff, },
...
impl Plugin for ItemsPlugin {
    fn build(&self, app: &mut App) {
        app .register_type::<ItemId>() .register_type::<PlayerTemporaryBuff>() .register_type::<ItemEffect>() .register_type::<ItemLibrary>() .register_type::<ExplosionEffect>() .register_type::<RetaliationNovaEffect>() .register_type::<TemporaryHealthRegenBuff>() .init_resource::<ItemLibrary>()
...
fn populate_item_library(mut library: ResMut<ItemLibrary>) {
    ...
    library.items.push(ItemDefinition { id: ItemId(6), name: "Unstable Cortex".to_string(), description: "Your projected thoughts have a chance to violently detonate on impact.".to_string(), effects: vec![ItemEffect::OnThoughtFragmentHitExplode { chance: 0.15, explosion_damage: 20, explosion_radius: 75.0, explosion_color: Color::rgba(1.0, 0.5, 0.2, 0.6), }], });
    library.items.push(ItemDefinition { id: ItemId(7), name: "Shroud of Retribution".to_string(), description: "When struck, has a chance to unleash a damaging psychic nova.".to_string(), effects: vec![ItemEffect::OnPlayerHitRetaliate { chance: 0.25, retaliation_damage: 30, retaliation_radius: 120.0, retaliation_color: Color::rgba(0.9, 0.1, 0.1, 0.5), }], });
    library.items.push(ItemDefinition { id: ItemId(8), name: "Soul Siphon Shard".to_string(), description: "Defeated foes have a 20% chance to grant brief, rapid health regeneration.".to_string(), effects: vec![ItemEffect::OnEnemyKillTrigger { chance: 0.20, effect: PlayerTemporaryBuff::HealthRegen { rate: 5.0, duration_secs: 3.0 }, }], });
...
fn apply_collected_item_effects_system( mut events: EventReader<ItemCollectedEvent>, mut player_query: Query<(&mut Player, Option<&mut ComponentHealth>)>, item_library: Res<ItemLibrary>, skill_library: Res<SkillLibrary>,) { // Added SkillLibrary
                ...
                for effect in &item_def.effects {
                    match effect {
                        ItemEffect::PassiveStatBoost { max_health_increase, speed_multiplier, damage_increase, xp_gain_multiplier, pickup_radius_increase, } => {
                            ...
                            if let Some(dmg_inc) = damage_increase { player.mind_fragment_damage_bonus += *dmg_inc; }
...
fn explosion_effect_system( mut commands: Commands, time: Res<Time>, mut explosion_query: Query<(Entity, &mut ExplosionEffect, &GlobalTransform, &mut Sprite, &mut Transform)>, mut enemy_query: Query<(Entity, &GlobalTransform, &mut Health), With<Enemy>>, asset_server: Res<AssetServer>, mut sound_event_writer: EventWriter<PlaySoundEvent>,) { for (explosion_entity, mut explosion, explosion_g_transform, mut sprite, mut vis_transform) in explosion_query.iter_mut() { explosion.timer.tick(time.delta()); let progress = explosion.timer.fraction(); let current_radius = explosion.radius_sq.sqrt(); vis_transform.scale = Vec3::splat(current_radius * 2.0 * progress); sprite.color.set_a(1.0 - progress); if explosion.timer.fraction() < 0.5 { let explosion_pos = explosion_g_transform.translation().truncate(); for (enemy_entity, enemy_g_transform, mut enemy_health) in enemy_query.iter_mut() { if explosion.already_hit_entities.contains(&enemy_entity) { continue; } let enemy_pos = enemy_g_transform.translation().truncate(); if enemy_pos.distance_squared(explosion_pos) < explosion.radius_sq { enemy_health.0 -= explosion.damage; spawn_damage_text(&mut commands, &asset_server, enemy_g_transform.translation(), explosion.damage, &time); sound_event_writer.send(PlaySoundEvent(SoundEffect::EnemyHit)); explosion.already_hit_entities.push(enemy_entity); } } } if explosion.timer.finished() { commands.entity(explosion_entity).despawn_recursive(); } } }
fn retaliation_nova_effect_system( mut commands: Commands, time: Res<Time>, mut nova_query: Query<(Entity, &mut RetaliationNovaEffect, &GlobalTransform, &mut Sprite, &mut Transform)>, mut enemy_query: Query<(Entity, &GlobalTransform, &mut Health), With<Enemy>>, asset_server: Res<AssetServer>, mut sound_event_writer: EventWriter<PlaySoundEvent>,) { for (nova_entity, mut nova, nova_g_transform, mut sprite, mut vis_transform) in nova_query.iter_mut() { nova.timer.tick(time.delta()); let progress = nova.timer.fraction(); let current_radius = nova.radius_sq.sqrt(); vis_transform.scale = Vec3::splat(current_radius * 2.0 * progress); sprite.color.set_a(1.0 - progress * progress); if nova.timer.fraction() < 0.3 { let nova_pos = nova_g_transform.translation().truncate(); for (enemy_entity, enemy_g_transform, mut enemy_health) in enemy_query.iter_mut() { if nova.already_hit_entities.contains(&enemy_entity) { continue; } let enemy_pos = enemy_g_transform.translation().truncate(); if enemy_pos.distance_squared(nova_pos) < nova.radius_sq { enemy_health.0 -= nova.damage; spawn_damage_text(&mut commands, &asset_server, enemy_g_transform.translation(), nova.damage, &time); sound_event_writer.send(PlaySoundEvent(SoundEffect::EnemyHit)); nova.already_hit_entities.push(enemy_entity); } } } if nova.timer.finished() { commands.entity(nova_entity).despawn_recursive(); } } }
fn temporary_health_regen_buff_system( mut commands: Commands, time: Res<Time>, mut buff_query: Query<(Entity, &mut TemporaryHealthRegenBuff, &Player, &mut ComponentHealth)>,) { for (entity, mut buff, player_stats, mut health_component) in buff_query.iter_mut() { buff.duration_timer.tick(time.delta()); if buff.duration_timer.finished() { commands.entity(entity).remove::<TemporaryHealthRegenBuff>(); } else { let regen_amount = buff.regen_per_second * time.delta().as_secs_f32(); health_component.0 = (health_component.0 as f32 + regen_amount).round() as i32; health_component.0 = health_component.0.min(player_stats.max_health); } } }