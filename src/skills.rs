...
pub enum SkillEffectType {
    ...
    AreaOfEffect { // For player-centered or targeted AoEs like Mind Shatter
    ...
    // PlaceTrap variant was removed
    SummonSentry { // For stationary, pulsing AoE summons
    ...
    FreezingNova { // New skill type for AoE damage + slow
        damage: i32,
        radius: f32,
        nova_duration_secs: f32, // How long the visual effect/damage window lasts
        slow_multiplier: f32,   // e.g., 0.5 for 50% speed reduction (speed * 0.5)
...
#[derive(Component)]
pub struct ActiveSkillAoEEffect {
    pub skill_id: SkillId,
    pub actual_damage_per_tick: i32,
    pub actual_radius_sq: f32,
...
pub struct PlayerBuffEffect {
    pub speed_multiplier_bonus: f32,
    pub fire_rate_multiplier_bonus: f32,
    pub duration_timer: Timer,
}

// PlacedTrap component definition removed

#[derive(Component, Debug, Reflect, Default)]
#[reflect(Component)]
pub struct FreezingNovaEffect {
    pub damage: i32,
    pub radius_sq: f32,
    pub lifetime_timer: Timer,
...
impl Plugin for SkillsPlugin {
    fn build(&self, app: &mut App) {
        app .register_type::<SkillId>() .register_type::<SkillEffectType>() .register_type::<SkillDefinition>() .register_type::<ActiveSkillInstance>() .register_type::<SkillLibrary>()
            // .register_type::<PlacedTrap>() // Removed
            .register_type::<FreezingNovaEffect>() // Registered
            ...
            .add_systems(Update, (
                ...
                player_buff_management_system,
                freezing_nova_effect_damage_system,
                // placed_trap_system removed
...
fn populate_skill_library(mut library: ResMut<SkillLibrary>) {
    ...
    // Abyssal Snare (SkillId(7)) and its PlaceTrap effect type are fully removed.
}
...
fn player_skill_input_system( mut commands: Commands, asset_server: Res<AssetServer>, mouse_button_input: Res<ButtonInput<MouseButton>>, keyboard_input: Res<ButtonInput<KeyCode>>, mut player_query: Query<(Entity, &mut Player, &Transform)>, skill_library: Res<SkillLibrary>, glyph_library: Res<GlyphLibrary>, mut sound_event_writer: EventWriter<PlaySoundEvent>,) {
    if let Ok((player_entity, mut player, player_transform)) = player_query.get_single_mut() {
        ...
        else if keyboard_input.just_pressed(KeyCode::KeyR) { skill_to_trigger_idx = Some(4); } // Input for 5th skill slot (index 4)
...
fn skill_projectile_collision_system(mut commands: Commands, mut skill_projectile_query: Query<(Entity, &GlobalTransform, &Damage, &mut SkillProjectile, &Sprite, &Velocity, &Lifetime)>, mut enemy_query: Query<(Entity, &GlobalTransform, &mut Health), With<Enemy>>, asset_server: Res<AssetServer>, time: Res<Time>, mut sound_event_writer: EventWriter<PlaySoundEvent>, skill_library: Res<SkillLibrary>, player_query: Query<&Player>, glyph_library: Res<GlyphLibrary>,) { let Ok(player) = player_query.get_single() else { return }; for (proj_entity, proj_g_transform, proj_damage, mut skill_projectile_data, proj_sprite, proj_velocity, proj_lifetime) in skill_projectile_query.iter_mut() { if skill_projectile_data.already_hit_by_this_projectile.len() > (skill_projectile_data.piercing_left + skill_projectile_data.bounces_left + 1) as usize { commands.entity(proj_entity).despawn_recursive(); continue; } let proj_pos = proj_g_transform.translation().truncate(); let proj_radius = proj_sprite.custom_size.map_or(5.0, |s| (s.x + s.y) / 4.0); let mut hit_an_enemy_this_frame = false; for (enemy_entity, enemy_gtransform, mut enemy_health) in enemy_query.iter_mut() { if skill_projectile_data.already_hit_by_this_projectile.contains(&enemy_entity) { continue; } let enemy_pos = enemy_gtransform.translation().truncate(); let enemy_radius = MINDLESS_SPAWN_SIZE.x / 2.0; if proj_pos.distance(enemy_pos) < proj_radius + enemy_radius { hit_an_enemy_this_frame = true; sound_event_writer.send(PlaySoundEvent(SoundEffect::EnemyHit)); enemy_health.0 -= proj_damage.0; spawn_damage_text(&mut commands, &asset_server, enemy_gtransform.translation(), proj_damage.0, &time); skill_projectile_data.already_hit_by_this_projectile.push(enemy_entity); if skill_projectile_data.piercing_left > 0 { skill_projectile_data.piercing_left -= 1; } else if skill_projectile_data.bounces_left > 0 { skill_projectile_data.bounces_left -= 1; let mut closest_new_target: Option<(Entity, f32)> = None; for (potential_target_entity, potential_target_gtransform, _health) in enemy_query.iter() { if skill_projectile_data.already_hit_by_this_projectile.contains(&potential_target_entity) { continue; } let distance_sq = potential_target_gtransform.translation().truncate().distance_squared(proj_pos); if distance_sq < (250.0 * 250.0) { if closest_new_target.is_none() || distance_sq < closest_new_target.unwrap().1 { closest_new_target = Some((potential_target_entity, distance_sq)); } } } if let Some((target_entity, _)) = closest_new_target { if let Ok((_t_ent, target_transform, _h)) = enemy_query.get(target_entity) { let direction_to_new_target = (target_transform.translation().truncate() - proj_pos).normalize_or_zero(); if let Some(active_skill_instance) = player.equipped_skills.iter().find(|s| s.definition_id == skill_projectile_data.skill_id) { if let Some(skill_def) = skill_library.get_skill_definition(skill_projectile_data.skill_id) { if let SkillEffectType::Projectile { speed, size, color, lifetime_secs, piercing, .. } = skill_def.effect { let mut chained_damage = proj_damage.0; let mut chained_piercing = piercing; let chained_bounces = skill_projectile_data.bounces_left; for glyph_opt in active_skill_instance.equipped_glyphs.iter() { if let Some(glyph_id) = glyph_opt { if let Some(glyph_def) = glyph_library.get_glyph_definition(*glyph_id) { match &glyph_def.effect { GlyphEffectType::AddedChaosDamageToProjectile { damage_amount } => { chained_damage += *damage_amount; } _ => {} } } } } commands.spawn(( SpriteBundle { texture: asset_server.load("sprites/skill_projectile_placeholder.png"), sprite: Sprite { custom_size: Some(size), color, ..default()}, transform: Transform::from_translation(proj_pos.extend(proj_g_transform.translation().z)) .with_rotation(Quat::from_rotation_z(direction_to_new_target.y.atan2(direction_to_new_target.x))), ..default() }, SkillProjectile { skill_id: skill_projectile_data.skill_id, piercing_left: chained_piercing, bounces_left: chained_bounces, already_hit_by_this_projectile: vec![enemy_entity], }, Velocity(direction_to_new_target * speed), Damage(chained_damage), Lifetime { timer: Timer::from_seconds(lifetime_secs, TimerMode::Once) }, Name::new(format!("ChainedProjectile_{}", skill_def.name)), )); } } } } } } commands.entity(proj_entity).despawn_recursive(); break; } else { commands.entity(proj_entity).despawn_recursive(); break; } } } if hit_an_enemy_this_frame && skill_projectile_data.piercing_left == 0 && skill_projectile_data.bounces_left == 0 {} } }
